...
def getStopWord(inputFile):
    stopWordList = readFile(inputFile, coding='utf-8').splitlines()
    return stopWordList
...
root_path = './../../data/text-corpus-directory-class'
# 分词，第一个是分词输入，第二个参数是结果保存的路径
segText(root_path+"/train/", root_path+"/segResult/")
bunchSave(root_path+"/segResult/", root_path+"/train_set.dat")  # 输入分词，输出分词向量
stopWordList = getStopWord(root_path+"/stop/stopword.txt")  # 获取停用词
getTFIDFMat(root_path+"/train_set.dat", stopWordList, root_path+"/tfidfspace.dat")  # 输入词向量，输出特征空间

# 训练集
segText(root_path+"/test/", root_path+"/test_segResult/")  # 分词
bunchSave(root_path+"/test_segResult/", root_path+"/test_set.dat")
getTestSpace(root_path+"/test_set.dat", root_path+"/tfidfspace.dat", stopWordList, root_path+"/testspace.dat")
bayesAlgorithm(root_path+"/tfidfspace.dat", root_path+"/testspace.dat")
